...
int evaluate_arg(const char *arg, parameters *p) {
    ...
    if (!strcmp(arg, "-f") || !strcmp(arg, "--force")) { p->force_overwrite = 1; return 1; }
...
int main(int argc, char **argv) {
    ...
    if (argc <= 1) {
        ...
        printf("  -f, --force    : Overwrite existing output VCD\n");
    ...
    if (snprintf(tmp_vcd_path, sizeof(tmp_vcd_path), "%s.tmp", final_vcd_path) >= (int)sizeof(tmp_vcd_path)) {
    ...
    if (check_disk_space(tmp_vcd_path, bin_size + VCD_HEADER_SIZE) != 0) {
        goto cleanup;
    }
    ...
    vcd_file = fopen(tmp_vcd_path, "wb");
    if (!vcd_file) {
        log_error("Cannot create temporary VCD output file", tmp_vcd_path);
        goto cleanup;
    }
    ...
    if (bytes_read > 0) {
        ...
        if (fwrite(outbuf, 1, bytes_read, vcd_file) != bytes_read) {
            log_error("Writing failure during initial block creation", tmp_vcd_path);
            goto cleanup;
        }
